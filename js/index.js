...
async function getsong(folder) {
    ...
    let s = await fetch(`http://127.0.0.1:5503/${folder}/`)
    let response = await s.text();

     console.log("check of response of getfolder 1:",response)
    let div = document.createElement("div")
    div.innerHTML = response;
    //sorting all a songs
    let sonlink = div.getElementsByTagName("a")
     console.log("check song list 2:",sonlink);
    songs = [];
...
async function displayalbum() {
    let s = await fetch(`http://127.0.0.1:5503/songs/`)
    let response = await s.text();
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (e.href.includes("/songs/")) {
            ...
            let s = await fetch(`http://127.0.0.1:5503/songs/${folder}/info.json`)
...
async function main() {
    //get song from any folder inside songs folder
     let a=await getsong("songs/ncs");
     let b=a;
     console.log("check fetch song",a);
